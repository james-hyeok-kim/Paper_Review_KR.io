...
* 26-Resnet + ConvMaxPool + Concate + MLP + MLP

1) 시각 제어 모델 (Visuomotor Models)이미지 입력을 사용하는 경우로, 다음과 같은 단계로 구성됩니다:
    1) 이미지 인코더 (CNN): 26-레이어 ResNet("ConvResNet") 또는 4-레이어 ConvMaxPool 구조를 사용하여 이미지에서 특징을 추출합니다.
    2) 특징 결합 (Late Fusion): 추출된 이미지 인코딩 벡터와 액션(Action) 벡터를 하나로 이어 붙입니다(Concatenate).
    3) 에너지 계산 (MLP): 결합된 벡터를 MLP에 통과시켜 최종적인 에너지 점수를 계산합니다.
2) 상태 기반 모델 (State-observation Models)
    1) 단일 MLP 구조: 별도의 복잡한 인코더 없이 관측 데이터와 액션 데이터를 결합하여 바로 MLP에 입력합니다.
    2) 출력: 마찬가지로 해당 (관측, 액션) 쌍에 대한 에너지 값을 출력합니다.

### 0.1 Summary

* 추론 방식: 에너지가 가장 낮은 행동이 곧 최선의 행동입니다. 이를 찾기 위해 추론 시점에 샘플링(DFO)이나 경사 하강법(Langevin) 같은 최적화 과정을 거칩니다.
* 학습 방식: 전문가의 행동은 에너지를 낮게, 그 외의 '가짜' 행동(음수 샘플)은 에너지를 높게 매기도록 InfoNCE 손실 함수를 통해 학습합니다. 

### 0.2. IBC의 주요 의의 (Significance)

1) 불연속성과 다중값 함수의 정복
    1) 기존 모델: 두 행동 사이의 '평균값'을 선택 
2) 극한의 정밀도 구현
    1) 1mm 수준의 정밀도
3) 보상 정보 없는 Offline RL 수준의 성능
    1) 보상(Reward) 정보를 전혀 사용하지 않고 오직 전문가의 시연 데이터만 학습

| 특징 | 명시적 BC (Explicit) | 암시적 BC (IBC/EBM) |
| :--- | :--- | :--- |
| **형태** | $\hat{a} = F_{\theta}(o)$ | $\hat{a} = \arg\min_a E_{\theta}(o, a)$ |
| **학습 방법** | MSE (평균 제곱 오차) 등 일반적인 지도 학습 | InfoNCE 손실 함수 및 음수 샘플링 기반 학습 |
| **추론 방식** | 단순 피드포워드 (Feed-forward) | 최적화(샘플링 또는 경사하강법)를 통한 암시적 회귀 |
| **추론 속도** | 매우 빠름 | 최적화 과정이 필요하여 상대적으로 느림 (계산량 많음) |
| **주요 장점** | 구현이 단순하고 튜닝이 쉬움 | 불연속적 행동, 멀티모달성, 고정밀 작업(1mm 단위)에 강함 |
