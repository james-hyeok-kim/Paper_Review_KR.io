...
| 전문가 <br> 구분 | 담당 데이터 <br> (Token Routing) | 설명 |
| :---: | :---: | :--- |
...
* 1블록 (이미지/언어): 기존 VLM 지식을 유지하기 위해 뒤에 나올 로봇 상태/행동을 참조하지 못하게 차단합니다.
* 2블록 (로봇 상태 $q_t$ ): 이전 이미지/언어 정보는 참조하지만, 다음에 나올 행동 토큰을 참조하지 않으므로 KV 캐싱이 가능해져 추론 속도가 빨라집니다.
* 3블록 (행동 토큰 $A_t^{\tau}$ ): 앞에 있는 모든 블록 정보(이미지, 언어, 로봇 상태)를 참조하여 정밀한 움직임을 생성합니다.
