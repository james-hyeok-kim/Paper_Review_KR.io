...
HSTU 한 레이어의 포워드 패스를 입력 토큰 시퀀스 $X \in \mathbb{R}^{n \times d}$ 기준으로 단계별로 정리한다.

---

```
입력 시퀀스 X  [n × d]
(각 토큰 = 아이템 임베딩 + 사용자 행동 임베딩 + 시간 정보)
        │
        ▼
┌──────────────────────────────────────────┐
...
└──────────────────────────────────────────┘
        ...
        ▼
┌──────────────────────────────────────────┐
│  Step 2. Pointwise Projection (Eq. 1)
│  단일 선형 변환 → SiLU 활성화 → Split
│  출력: U, V, Q, K  [n × h × d_v/d_qk]
└──────────────────────────────────────────┘
        │
        ▼
┌──────────────────────────────────────────┐
│  Step 3. Spatial Aggregation (Eq. 2)
│  A = ϕ₂(QKᵀ + r_p,t)
│  어텐션 맵에 위치·시간 편향 r 추가
│  softmax 없음 → pointwise 정규화
│  출력: A(X)V(X)  [n × h × d_v]
└──────────────────────────────────────────┘
        │
        ▼
┌──────────────────────────────────────────┐
│  Step 4. Pointwise Transformation (Eq.3)
│  LayerNorm → ⊙ U(X) → 선형 변환 f₂
│  U(X)가 게이팅 역할 (SwiGLU 변형)
│  출력: Y(X)  [n × d]
└──────────────────────────────────────────┘
        │
        ▼
...
**Step 1. Feature Tokenization — 이종 피처를 하나의 시계열로**

DLRM에서 "범주형 피처(아이템 ID, 크리에이터 ID)"와 "수치형 피처(클릭률, 가중 카운터)"는 전혀 다른 경로로 처리된다. GR에서는 이 모든 피처를 하나의 통합 시계열로 병합한다.

* 메인 시계열: 사용자가 실제로 참여한 아이템 시퀀스 $(\Phi_0, a_0, \Phi_1, a_1, \ldots)$. 아이템($\Phi_i$)과 그에 대한 사용자 행동($a_i$, 예: 좋아요/건너뛰기/완시청)을 **교대(interleaving)** 로 배치한다. 이 배치 덕분에 타깃 아이템 $\Phi_{i+1}$이 시퀀스 내부에서 직접 어텐션에 참여하므로, "어떤 아이템을 보여줬을 때 이 사용자가 클릭할까?"라는 **타깃 인식(target-aware) 추론**이 단일 포워드 패스 안에서 이루어진다.
* 보조 시계열: 팔로우 중인 크리에이터, 가입 커뮤니티 등 천천히 변하는 피처는 연속 구간당 최초 항목만 남긴 뒤 메인 시계열에 삽입한다(시퀀스 길이 증가 최소화).
* 수치형 피처: CTR 같은 비율 피처는 완전 순차화가 불가능하지만, 해당 피처가 집계되는 범주형 특징(아이템 주제, 지역)은 이미 시퀀스에 포함되어 있어 시퀀스가 충분히 길어지면 모델이 자체 학습으로 근사할 수 있다.

**Step 2. Pointwise Projection — 한 번의 행렬 연산으로 Q·K·V·U 동시 생성**

$$U(X),\, V(X),\, Q(X),\, K(X) = \text{Split}\!\left(\phi_1(f_1(X))\right)$$

여기서 $f_1(X) = W_1 X + b_1$은 단일 선형 변환, $\phi_1$은 SiLU 활성화 함수다. Transformer가 Q·K·V를 각각 별도 행렬로 계산하는 것과 달리, HSTU는 4개의 출력을 **하나의 행렬 연산 후 분할(split)** 하여 커널 융합(kernel fusion)을 가능하게 한다. $U(X)$는 Step 4에서 게이팅 역할을 하는 "조건부 가중치"다.

**Step 3. Spatial Aggregation — softmax 없는 포인트와이즈 어텐션**

$$A(X)V(X) = \phi_2\!\left(Q(X)K(X)^T + r^{ab}_{p,t}\right)V(X)$$

여기서 $r^{ab}_{p,t}$는 **상대 위치(p)와 상대 시간(t) 편향**을 동시에 인코딩하는 학습 가능한 바이어스다. 시퀀스 안에서 "두 이벤트가 몇 단계 떨어졌는지"와 "실제 시각 차이가 얼마나 되는지"를 별도로 인코딩하여, 시간 패턴에 민감한 추천(예: 최근 본 아이템에 더 강하게 반응)을 표현한다.

Transformer의 softmax 어텐션과 결정적으로 다른 점이 있다. softmax는 어텐션 가중치 합을 1로 정규화하기 때문에, 사용자가 특정 카테고리를 100번 본 것과 1번 본 것이 어텐션 출력에서 같은 크기로 취급될 수 있다. HSTU는 대신 **포인트와이즈 정규화(pointwise normalization)** 를 쓰므로, "이 카테고리에 얼마나 많이 반응했는가"라는 선호 **강도** 정보가 표현에 그대로 보존된다. 또한 비정적(non-stationary) 어휘 환경에서 softmax가 불안정해지는 문제도 피할 수 있다.

Stochastic Length(SL) 적용 시 훈련 중 긴 시퀀스를 확률적으로 서브샘플링하여, 이 단계의 $O(n^2 d)$ 복잡도를 $O(N^\alpha d)$로 낮춘다($\alpha \in (1, 2]$, 길이 4096 기준 $\alpha=1.6$에서 80.5% 스파시티 달성).

**Step 4. Pointwise Transformation — 게이팅으로 조건부 계산**

$$Y(X) = f_2\!\left(\text{Norm}\!\left(A(X)V(X)\right) \odot U(X)\right)$$

여기서 $\odot$는 원소별 곱(element-wise product), $\text{Norm}$은 LayerNorm이다. Step 2에서 미리 계산해 둔 $U(X)$가 어텐션 출력을 원소별로 스케일 조정한다. SiLU가 $U(X)$에 적용되어 있으므로 이 식은 **SwiGLU의 변형**으로 해석 가능하다. 동시에, $U(X)$가 각 토큰마다 다른 크기로 어텐션 출력을 증폭·억제하기 때문에 Mixture of Experts의 "조건부 라우팅"처럼 동작한다. 즉 동일 레이어가 다양한 사용자 프로필에 대해 자동으로 다른 처리 경로를 선택하는 효과를 낸다.

이 설계 덕분에 HSTU는 Transformer 대비 레이어당 활성화 메모리를 **33d → 14d로 절감**하여, 동일 GPU 메모리로 2배 이상 깊은 네트워크를 구성할 수 있다.

**Step 5. 잔차 합산 및 레이어 반복**

$Y(X) + X$로 잔차 연결 후 동일 구조의 레이어를 $L$번 반복한다. 최종 레이어 출력의 각 위치 $i$ 토큰 표현은 랭킹(행동 예측 헤드 → 멀티태스크 loss) 또는 검색(임베딩 유사도 → MIPS) 예측에 사용된다.
