...
#### 0.6.5. R / Q / p 역할 상세 (Block Diagram)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ┌──────────────────────┐  ┌──────────────────────────┐  ┌────────────────────────┐
  │   Reward  R(z, a)    │  │   Value  Q(z, a)          │  │  Policy Prior  p(z)    │
  │                      │  │                          │  │                        │
  │  입력: 상태 + 행동    │  │  입력: 상태 + 행동        │  │  입력: 상태만           │
  │  출력: 숫자 (점수)    │  │  출력: 숫자 (점수)        │  │  출력: 행동             │
  │                      │  │                          │  │                        │
  │  "지금 이 행동이      │  │  "지금부터 게임 끝까지    │  │  "지금 어떤 행동을      │
  │   얼마나 좋았나?"     │  │   총 얼마나 받을 수 있나" │  │   해야 하나?"           │
  │                      │  │                          │  │                        │
  │  한 스텝 즉각 보상    │  │  R을 미래까지 모두 합산   │  │  Q를 최대화하는 행동    │
  │  → 평가              │  │  → 평가 (더 먼 미래까지)  │  │  → 행동 제안            │
  └──────────────────────┘  └──────────────────────────┘  └────────────────────────┘

  Q와 R의 관계:
  Q(z_t, a_t) = R(z_t, a_t)              ← 지금 한 스텝 보상
              + γ  · R(z_{t+1}, a_{t+1}) ← 다음 스텝
              + γ² · R(z_{t+2}, a_{t+2}) ← 그 다음 스텝
              + γ³ · ...                  ← 계속 (무한 합산)

  → Q는 R의 누적 합이다. R이 "지금 점수", Q는 "총 점수"

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  [B] 학습 시 — R, Q, p의 관계와 화살표
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Replay Buffer에서 (z_t, a_t, r_t, z'_t) 샘플
       │
       ├─ [R 학습]
       │    R(z_t, a_t) ≈ r_t    ← 실제 보상과 비교, cross-entropy 손실
       │    p는 R 학습에 직접 개입하지 않음
       │
       ├─ [Q 학습]  ← p가 화살표로 개입
       │
       ...
       │                     │   ← "다음 상태 z'_t에서 어떤 행동?"
       │                     ▼
       │    q_t = r_t + γ · Q̄(z'_t,  a'  )    (TD-target)
       │                          ↑
       │                    p가 고른 행동
       │
       │    Q(z_t, a_t) ≈ q_t     ← 이 target에 가까워지도록 학습
       │
       └─ [p 학습]  ← Q가 화살표로 개입
            maximize  α · Q(z_t,  p(z_t) )  −  β · H(p(·|z_t))
                              ↑       ↑
                          Q가 평가   p가 고른 행동    엔트로피 유지
  ...
  p → Q : p가 TD-target 계산에 쓸 행동을 제공
  Q → p : p가 Q값을 높이도록 학습 (p의 손실함수에 Q 포함)
  p → R : 직접 화살표 없음 (R은 실제 보상 r_t와만 비교)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  [C] 계획(MPPI) 시 — R, Q, p가 협력하는 방식
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  현재 상태 z_t
       ...
       └─ Gaussian 랜덤 샘플 488개 ──────────────────────────┐ │
                                                             ▼ ▼
                                              512개 후보 시퀀스 [a_t,...,a_{t+H}]
                                                               │
                    ┌──────────────────────────────────────────┘
                    │  각 후보 시퀀스 평가 (latent rollout):
                    │
                    │  스텝 h = 0, 1, ..., H-1:
                    │    z_h ──►[d(z_h,a_h)]──► z_{h+1}   (dynamics)
                    │    z_h ──►[R(z_h,a_h)]──► r̂_h       ← R 담당 (즉각 보상)
                    │
                    │  스텝 H (horizon 끝):
                    │    z_H ──►[Q(z_H,a_H)]──► 이후 총합  ← Q 담당 (먼 미래)
                    │
                    ▼
       ┌────────────────────────────────────────────────────────────┐
       │  총 기대 return = Σ_{h=0}^{H-1} γ^h · R(z_h, a_h)        │
       │                 +               γ^H · Q(z_H, a_H)          │
       │                 ↑ p가 제안한 행동들이 R, Q로 평가됨         │
       └────────────────────────────────────────────────────────────┘

  계획 시 화살표:
    p ──► (행동 후보) ──► R  (각 스텝 보상 평가)
                     └──► Q  (H 이후 보상 평가)

  return이 높은 시퀀스 위주로 Gaussian (μ, σ) 갱신 → 반복 6회
  최종 μ에서 a_t 샘플 → 환경에 실행
```
...
| | 질문 | 입력 | 출력 | 계획 시 담당 |
|---|---|---|---|---|
| **R** | 지금 이 행동이 얼마나 좋나? | 상태 + 행동 | 숫자 | H 스텝 안 보상 합산 |
| **Q** | 지금부터 끝까지 총 얼마? | 상태 + 행동 | 숫자 | H 이후 보상 요약 |
| **p** | 어떤 행동을 해야 하나? | 상태만 | 행동 | 유망한 후보 제안 + TD-target 행동 결정 |
