...
θ
​
 (x)를 직접 계산하는 대신, **ELBO (Evidence Lower Bound)**를 최대화해서 근사한다.
...
ϕ
​
 (z∣x): encoder 또는 approximate posterior
...
2.3 Variational Inference in DDPM
🔹 Diffusion에서는 어떻게 사용되는가?
DDPM에서는 latent variable 
𝑧
z 대신, 노이즈가 점진적으로 추가된 상태 
𝑥
𝑡
x 
t
...
t
​
 )는 learnable한 분포로써 학습됨.

이 구조가 VAE와 유사한 변분 추정 구조를 가짐
→ 따라서 ELBO 기반 loss로 학습 가능함
