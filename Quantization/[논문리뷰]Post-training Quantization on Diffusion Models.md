...
#### 1. 보정 데이터 (Calibration Data)

• PTQ는 대규모 학습 데이터셋 대신 보정 데이터라고 불리는 소량의 샘플(예: 100개 내외)을 사용하여 양자화 파라미터를 설정합니다.  

#### 2. 최신 기술 트렌드
• 초기 PTQ 연구는 주로 가중치나 활성화 값의 통계적 분포(최소-최대값, 평균 등)를 기반으로 범위를 결정했습니다.  
• 최신 연구인 BRECQ나 AdaRound 등은 가중치를 반올림할 때 생기는 재구성 오류(reconstruction error)를 최적화하여 양자화 성능을 획기적으로 높이고 있습니다.  

#### 3. 디퓨전 모델에서의 중요성:
• 디퓨전 모델은 추론 시 수십 번에서 수백 번의 반복 연산이 필요하기 때문에 연산량이 매우 많습니다. 따라서 PTQ를 통해 모델을 경량화하는 것은 추론 속도를 높이고 메모리 사용량을 줄이는 데 결정적인 역할을 합니다
