...
<p align = 'center'>
<img width="708" height="596" alt="image" src="https://github.com/user-attachments/assets/d958a6b8-ad90-433e-b2bd-ef4df26f09de" />
</p>

* 실험은 4개의 실제 벤치마크(DrealSR, RealSR, RealLR200, RealLQ250)에서 수행되었습니다.

* W4A6 설정 (가중치 4비트, 활성화 6비트)
    * 활성화 정밀도가 충분히 높아 VaTMP를 끄고도 모든 데이터셋과 지표에서 Full-Precision(FP) 모델에 매우 근접한 성능을 보여주었습니다.
    * 특이점은 이 수준의 성능을 내기 위해 별도의 교정(Calibration) 데이터가 전혀 필요하지 않았다는 점입니다.

* W4A4 설정 (가중치 4비트, 활성화 4비트)
    * 이 설정에서는 VaTMP(시간적 혼합 정밀도)를 활성화하여 오류를 효과적으로 억제했습니다.
    * 기존 방식들이 성능 저하를 겪는 것과 대조적으로, Q-DiT4SR은 가장 우수한 전체 성능을 유지하며 낮은 정밀도에서도 견고함을 입증했습니다.

#### 2. 시각적 비교 (Visual Comparison)

<p align = 'center'>
<img width="712" height="604" alt="image" src="https://github.com/user-attachments/assets/1ea7138f-0849-49fd-a97d-8c6be4010089" />
</p>

* 디테일 복원: 다른 베이스라인 모델들에 비해 더 날카로운 구조와 충실한 고주파 세부 정보를 재구성했습니다.
* 선명도 및 질감: 에지가 더 명확하고 텍스처가 풍부하게 표현되었습니다.
* FP 모델과의 유사성: 공격적인 W4A4 설정에서도 원본 FP 모델과 비교했을 때 인지적인 차이가 미미할 정도로 높은 품질을 유지했습니다.

#### 3. 지표와 실제 화질 간의 불일치 관찰

* 노이즈와 점수: Q-Diffusion이나 EfficientDM 같은 일부 방식은 시각적으로 노이즈가 심함에도 불구하고 MANIQA와 같은 특정 비참조 지표에서 비정상적으로 높은 점수를 받기도 했습니다.
* 결론: 이는 현재의 화질 평가 지표(IQA)와 실제 인간이 느끼는 인지적 품질 사이에 괴리가 있음을 시사하며, 향후 더 적절한 평가 기준이 필요함을 강조합니다.


### 4.3. Ablation Study

#### 1. SVD-L의 랭크 예산 (Rank Budget of SVD-L)

* 실험에서 전역 분기인 SVD-G의 랭크는 32로 고정하고 SVD-L의 랭크만 변화시켰습니다.

* 품질-효율성 트레이드오프: 랭크 예산을 4에서 8로 늘릴 때 가장 큰 성능 향상이 관찰되었으며, 그 이상(16, 32)으로 늘리면 이득이 미미하거나 오히려 부정적인 영향을 주기도 했습니다.

* 또한 랭크가 높아질수록 FLOPs와 파라미터 수가 단조 증가하므로, 최적의 효율을 위해 SVD-L의 랭크를 8로 설정했습니다.

#### 2. VaSMP의 효과

* 가중치 비트 할당 전략인 VaSMP의 성능을 W4A6 설정에서 평가했습니다.
* H-SVD의 기여: 기본 모델에 H-SVD를 통합하는 것만으로도 모든 지표에서 일관된 성능 향상을 보였습니다.
* 기존 MP와의 비교: 전역적인 MSE 목적 함수를 사용하는 나이브한 혼합 정밀도(MP) 방식은 레이어 간 민감도 차이를 포착하지 못해 이득이 제한적이었습니다.
* VaSMP의 우수성: 가중치 분산 통계를 가이드로 사용하는 VaSMP는 레이어 간 가변성을 명확히 모델링하여 모든 IQA 지표에서 최고 성적을 거두었습니다.

#### 3. VaTMP의 효과

* 가장 공격적인 W4A4 환경에서 활성화 비트 스케줄링인 VaTMP의 유효성을 검증했습니다.
* 시너지 효과: 이미 H-SVD와 VaSMP를 결합하여 낮은 활성화 정밀도에서도 성능을 개선한 상태에서, VaTMP를 추가하면 추가적인 성능 이득이 발생합니다.
* 시각적 증거: VaTMP를 적용했을 때 동일한 비트 예산 내에서도 미세한 질감과 국부 구조를 훨씬 더 잘 보존하는 것으로 나타났습니다.
* 비트 할당 패턴: 분석 결과, VaTMP는 시간에 따른 민감도 변화에 맞춰 정밀도를 동적으로 조절하는 계단식 스케줄링 패턴을 보였습니다.


| 용어 | 범위 | 주요 역할 | 보존하는 정보 |
| :--- | :---: | :--- | :--- |
| H (Hadamard) | 전체 | 이상치 제거 및 분포 균일화 | 수치적 안정성 |
| SVD-G | 전역 | 행렬 전체의 저계수 근사 | 이미지의 큰 구조 및 형태 |
| SVD-L | 국부 | 블록 단위의 Rank-1 근사 | 미세한 텍스트 및 고주파 디테일 |
| H-SVD (Hierarchical) | 통합 | G와 L을 계층적으로 결합 | 전반적인 화질 및 선명도 극대화 |
...
* 최초의 전용 프레임워크: DiT 기반 Real-ISR의 효율적인 배포를 위해 맞춤 설계된 최초의 사후 훈련 양자화(PTQ) 프레임워크인 Q-DiT4SR을 제안했습니다.
* 세부 구조 보존: 저비트 환경에서도 미세한 구조를 유지하기 위해, 전역 저계수 분기와 국부 Rank-1 분기를 통합한 H-SVD 가중치 재구성 전략을 개발했습니다.
* 지능형 비트 할당: 데이터 없이 레이어별 가중치 비트를 할당하는 VaSMP와 시간 단계별 활성화 정밀도를 조절하는 VaTMP를 통해 시공간적인 혼합 정밀도를 실현했습니다.
* SOTA 성능 달성: 광범위한 실험을 통해 W4A6 및 W4A4 설정 모두에서 기존 방식들을 뛰어넘는 최첨단(SOTA) 성능을 입증했습니다.
* 실용적 기여: 이번 연구가 자원이 제한된 환경에서도 고성능 DiT 백본을 실질적으로 활용할 수 있게 하는 중요한 발판이 되기를 기대하고 있습니다.
