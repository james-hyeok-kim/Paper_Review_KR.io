...
* 핵심 과제
    * 입력 드리프트: 상류(Upstream) LLM 백본이 양자화되면서 발생하는 미세한 섭동이 하류의 DiT 액션 정책으로 전축됩니다.
    * 정밀도 손실: DiT는 실제 로봇 제어를 위한 정밀한 토큰을 생성해야 하므로, 아주 작은 반올림 오차나 스케일 불일치도 큰 제어 오류로 이어집니다.
    * 수학적 분석: 분석 결과, 양자화 오차는 어텐션의 로그 온도($s_{q}s_{k}$)와 잔차 스트림 에너지( $s_{v}s_{o}$ )라는 두 가지 결정적 요인을 왜곡시키는 것으로 나타났습니다.
...
### 1. 세계 최초의 VLA 전용 PTQ 프레임워크

### 2. 핵심 기술의 통합적 설계

* 선택적 레이아웃(Selective Layout): 언어 백본 전체와 디퓨전 트랜스포머(DiT)의 피드포워드(MLP) 블록은 정수형으로 양자화하여 메모리를 절감하는 전략을 취했습니다.
* 정밀도 유지: 성능에 가장 결정적인 영향을 미치는 어텐션 프로젝션(Attention Projections) 레이어들은 부동 소수점 상태로 유지하여 오차 확산을 방지했습니다.
* 경량 보정 기술: ATM(온도 매칭)과 OHB(출력 밸런싱)라는 두 가지 가벼운 스칼라 보정치를 도입하여 저비트 추론 환경에서도 어텐션 분포와 출력 에너지를 안정화했습니다.

### 3. 주요 성과: 효율성과 정확도의 동시 달성

* 메모리 혁신: 양자화된 모듈에서 약 70%의 상대적 메모리 절감을 달성.
* 성능 향상: 단순히 메모리만 줄인 것이 아니라, 보정 기술을 통해 원본 모델보다 더 높은 작업 성공률을 기록하며 정확도 면에서도 우수함을 입증.

### 4. 미래 전망 및 실용적 가치

* 저전력 배포: QuantVLA는 전력 예산이 엄격한 환경에서도 신뢰할 수 있는 장기(Long-horizon) 동작 생성을 가능하게 하는 토대를 마련했습니다.
* 범용성: 다양한 모달리티와 모델 구조에 대해 강건함(Robustness)을 보여주었으며, 향후 고성능 로봇 지능의 저비트 배포를 위한 표준적인 방법론이 될 것으로 기대됩니다.
