...
* 구체적인 예시
    * 원본 데이터 (가중치 서브 벡터)
        ...
        * $w_2 = [0.49, 0.51]$
        * $w_3 = [0.11, 0.89]$
    * 코드북 생성이 데이터들을 대표할 수 있는 2개의 대표값(코드워드)을 정합니다 (보통 K-Means 알고리즘 사용).
        * ID 0번: $[0.1, 0.9]$
        * ID 1번: $[0.5, 0.5]$
     * 할당 (Assignment) 과정이제 각 원본 데이터와 가장 유사한 코드북 ID를 매칭합니다.
        * $w_1$은 $[0.1, 0.9]$와 가장 비슷하므로 ID 0 할당
        * $w_2$는 $[0.5, 0.5]$와 가장 비슷하므로 ID 1 할당
        * $w_3$은 $[0.1, 0.9]$와 가장 비슷하므로 ID 0 할당
     * 결과 (압축 완료)
         * 이제 모델은 $[0.12, 0.88]$ 같은 긴 소수점 숫자들을 저장하는 대신, 훨씬 가벼운 번호인 [0, 1, 0]만 저장하면 됩니다.
         * 나중에 모델을 쓸 때는 이 번호를 보고 코드북에서 다시 숫자 묶음을 꺼내와 원래 가중치를 복구합니다.
