...
* 기술적 개선
    * Scale-aware AdaRound: 기존 정수 기반의 적응형 반올림(AdaRound) 기술을 FPQ에 맞게 확장 및 일반화하여 가중치 보정의 성능을 높였습니다.
        * 정수(INT) 방식: 모든 숫자의 간격이 일정
        * 부동소수점(FP) 방식: 숫자가 0에 가까우면 촘촘하고, 멀어지면 간격이 넓어짐
        * 간격이 넓은 곳(큰 숫자)에서는 값이 확 변하고, 간격이 좁은 곳(작은 숫자)에서는 값이 조금 변함
        * 똑같은 느낌으로 돌아가도록 설계
        * 다이얼의 조절 값( $V'$ )을 해당 숫자의 간격 크기( $s$ )로 나누어 
